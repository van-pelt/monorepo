...
type Service interface {
	...
	// TransferTx runs in the caller-provided transaction so it can be
	// composed atomically with consumers.Dedup's processed_events insert.
	TransferTx(ctx context.Context, q postgres.Querier, fromID, toID uuid.UUID, amount int64) error
}
...
// TransferTx moves funds between two accounts using the caller's tx. The
// account module's payment.created handler wraps this in consumers.Dedup,
// which provides the tx so the transfer commits atomically with the
// processed_events mark.
func (s *service) TransferTx(ctx context.Context, q postgres.Querier, fromID, toID uuid.UUID, amount int64) error {
